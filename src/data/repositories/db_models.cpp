...
bool ensureDbOpen(const QSqlDatabase &db, const char *context)
{
    if (db.isOpen())
        return true;
    qDebug() << context << "БД не открыта!";
    return false;
}

ModelInfo rowToModelInfo(const QSqlQuery &q)
{
    ModelInfo m;
...
QStringList loadModelNames()
{
    ...
    if (!ensureDbOpen(db, "loadModelNames:")) {
        return list;
    }
...
QVector<ModelInfo> loadModelList()
{
    ...
    QSqlDatabase db = openMainDb();
    if (!ensureDbOpen(db, "loadModelList:")) {
        return list;
    }
    ...
    while (q.next())
        list.push_back(rowToModelInfo(q));
...
{
    QSqlDatabase db = openMainDb();
    if (!ensureDbOpen(db, "deleteModelByName:")) {
        return false;
    }
