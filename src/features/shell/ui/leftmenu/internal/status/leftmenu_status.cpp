...
constexpr int kUpcomingMaintenanceUiMoreStep = 50;
...
void leftMenu::updateMaintenanceOverdueCounter()
{
    if (!maintenanceOverdueCounter_)
        return;

    const int overdue = loadSystemStatus().error;
    maintenanceOverdueCounter_->setText(QStringLiteral("Просрочено: %1").arg(overdue));
    if (overdue > 0) {
        maintenanceOverdueCounter_->setStyleSheet(QString(
            "background:#FEE2E2;color:#DC2626;font-family:Inter;font-size:%1px;font-weight:800;"
            "border-radius:%2px;padding:%3px %4px;"
        ).arg(s(12)).arg(s(10)).arg(s(4)).arg(s(10)));
    } else {
        maintenanceOverdueCounter_->setStyleSheet(QString(
            "background:#E5E7EB;color:#6B7280;font-family:Inter;font-size:%1px;font-weight:800;"
            "border-radius:%2px;padding:%3px %4px;"
        ).arg(s(12)).arg(s(10)).arg(s(4)).arg(s(10)));
    }
}

void leftMenu::updateMainWindowTitle()
{
    QWidget *top = window();
    if (!top)
        return;
    ...
    if (db.isOpen()) {
        ...
        if (q.exec(QStringLiteral("SELECT 1")) && q.next())
            dbOk = true;
        if (q.exec(QStringLiteral("SELECT COUNT(*) FROM agv_list")) && q.next())
            agvCount = q.value(0).toInt();
    }

    const QString status = dbOk ? QStringLiteral("подключено") : QStringLiteral("нет связи");
    top->setWindowTitle(QStringLiteral("VapManager — %1 AGV — %2 (%3)")
                            .arg(agvCount)
                            .arg(host)
                            .arg(status));
}

void leftMenu::refreshConnectionUi()
{
    updateAgvCounter();
    updateMaintenanceOverdueCounter();
    updateMainWindowTitle();
}

void leftMenu::tryReconnectDatabase()
{
    // Переподключение не чаще раза в 30с — иначе при сетевых «бульканиях»
    // на удалённых клиентах будем постоянно дёргать соединение.
    static qint64 lastAttemptMs = 0;
    const qint64 now = QDateTime::currentMSecsSinceEpoch();
    if (lastAttemptMs != 0 && (now - lastAttemptMs) < 30000)
        return;

    // Важно: локальную копию QSqlDatabase и QSqlQuery уничтожаем ДО вызова
    // removeMainConnection() (внутри reconnectWithHost). Иначе QPSQL-драйвер
    // падает: соединение удаляется, пока на него ещё есть ссылки в стеке.
    bool ok = false;
    {
    ...
    lastAttemptMs = now;
    QString err;
    if (reconnectWithHost(getDbHost(), &err)) {
        refreshConnectionUi();
...
void leftMenu::updateUpcomingMaintenance()
{
    ...
    int uiSlotsLeft = upcomingMaintenanceUiLimit_;
