...
TaskChatThread rowToThread(const QSqlQuery &q)
{
    ...
    if (q.record().count() > 10) {
        t.lastMessageAt = q.value(9).toDateTime();
        const QString rawBody = q.value(10).toString();
        if (!rawBody.isEmpty()) {
            QString preview = ChatMessageCrypto::decrypt(rawBody).trimmed();
            preview.remove(QRegularExpression(QStringLiteral("\\s+")));
            if (preview.size() > 90)
...
bool reopenThread(int threadId, QString &error)
{
    ...
    QSqlQuery q(db);
    q.prepare("UPDATE task_chat_threads SET closed_at = NULL, closed_by = NULL WHERE id = :id");
    q.bindValue(":id", threadId);
    if (!q.exec()) {
        error = q.lastError().text();
        return false;
...
QVector<TaskChatThread> getThreadsForUser(const QString &username)
{
    ...
    q.prepare(
        ...
        "lm.last_msg_at, lm.last_msg_body "
        "FROM task_chat_threads t "
        "LEFT JOIN LATERAL ("
        "  SELECT m.created_at AS last_msg_at, m.message AS last_msg_body "
