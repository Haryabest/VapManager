    ...
    : QFrame(parent), s(scale)
{
    ...
    connect(deleteBtn, &QPushButton::clicked, this, [this](){
        ...
        dlg.setStyleSheet(
            ...
            "QLabel { background: transparent; font-family: Inter; font-size: 18px; font-weight: 800; color: #1A1A1A; }"
            "QCheckBox { background: transparent; font-family: Inter; font-size: 16px; color: #1A1A1A; }"
            "QPushButton { font-family: Inter; font-size: 16px; font-weight: 800; border-radius: 8px; padding: 6px 14px; }"
            "QPushButton#ok { background: #FF3B30; color: white; }"
            "QPushButton#ok:hover { background: #E13228; }"
            "QPushButton#cancel { background: #E6E6E6; color: black; }"
            "QPushButton#cancel:hover { background: #D5D5D5; }"
        );
        ...
        for (int i = 0; i < layout->count(); ++i) {
            ...
            if (agvItem) {
                // Показываем ID пользователю
                QCheckBox *cb = new QCheckBox(agvItem->agvId(), &dlg);

                // Но удалять будем по реальному ID
                cb->setProperty("db_id", agvItem->agvId());
        ...
        if (dlg.exec() == QDialog::Accepted) {
            ...
            for (auto cb : boxes) {
                if (cb->isChecked()) {
                    ...
                    QString id = cb->property("db_id").toString();
                    removeAgvById(id);
                }
            }
...
QVector<AgvInfo> ListAgvInfo::loadAgvList()
{
    ...
    q.prepare("SELECT agv_id, model, serial, status, alias, kilometers, blueprintPath, lastActive "
              "FROM agv_list ORDER BY created_at DESC");
...
bool deleteAgvFromDb(const QString &id)
{
...
void ListAgvInfo::removeAgvById(const QString &id)
{
    ...
    // 3) Перезагружаем список из базы
    QVector<AgvInfo> agvs = loadAgvList();
    rebuildList(agvs);
