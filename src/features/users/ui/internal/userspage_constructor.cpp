    ...
    : QWidget(parent), s(scale)
{
    ...
    showRecoveryKeys_ = (currentRole == QStringLiteral("tech"));

    avatarWatcher_ = new QFutureWatcher<QHash<QString, QByteArray>>(this);
    connect(avatarWatcher_, &QFutureWatcher<QHash<QString, QByteArray>>::finished,
            this, [this]() {
        if (!avatarWatcher_ || pendingAvatarGeneration_ != loadGeneration_)
            return;

        const QHash<QString, QByteArray> avatars = avatarWatcher_->result();
        for (auto it = avatars.constBegin(); it != avatars.constEnd(); ++it) {
            UserItem *item = userItems_.value(it.key());
            if (!item)
                continue;

            QPixmap pm;
            if (pm.loadFromData(it.value()))
