...
bool AgvSettingsPage::completeTaskNow(const QString &taskId, const AgvTask &task, bool showResultDialog)
{
...
void AgvSettingsPage::completeAllTasks()
{
    if (currentAgvId.trimmed().isEmpty()) {
        QMessageBox::warning(this, "Задачи", "AGV не выбран.");
        return;
    }

    const QString currentUser = AppSession::currentUsername();
    const QString curRole = getUserRole(currentUser);
    const bool isOffline = (originalStatus.trimmed().toLower() == QStringLiteral("offline"));
    if (isOffline) {
        QMessageBox::warning(this, "Задачи", "AGV в статусе offline — проведение задач недоступно.");
        return;
    }
    if (curRole != QStringLiteral("admin") && curRole != QStringLiteral("tech")) {
        QMessageBox::warning(this, "Задачи", "Провести все задачи могут только admin или tech.");
        return;
    }
    ...
    for (int i = 0; i < tasksLayout->count(); ++i) {
        ...
        if (!w)
            continue;
        const QString taskId = w->property("task_id").toString();
        if (!taskId.isEmpty())
            taskIds.append(taskId);
    }
    if (taskIds.isEmpty()) {
        QMessageBox::information(this, "Задачи", "Нет задач для проведения.");
        return;
    }
    ...
    const auto reply = QMessageBox::question(
        this,
        "Провести все задачи",
        QString("Провести все задачи (%1 шт.) для AGV %2?").arg(taskIds.size()).arg(currentAgvId),
    ...
    for (const QString &taskId : taskIds) {
        const AgvTask task = loadTaskById(taskId);
        if (task.id.isEmpty()) {
            ++skipped;
            continue;
        }
        if (completeTaskNow(taskId, task, false))
            ++done;
        else
            ++skipped;
    }
    ...
    QMessageBox::information(
        this,
        "Задачи",
        QString("Проведено: %1\nПропущено: %2").arg(done).arg(skipped));
}
