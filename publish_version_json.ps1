param(
    ...
    [string]$Notes = "Build 161: no full UI rebuild when virtual keyboard opens/closes on tablets (scale from width only, not height). Build 160: fixes chat input on tablets. Build 159: chat context menu, advanced search, tray update notification."
)

$updDir = Join-Path $RepoRoot "updates"
$outDir = Join-Path $updDir "files"

if (-not (Test-Path $outDir)) {
    Write-Host "[ERROR] updates\files not found: $outDir"
    exit 1
}
