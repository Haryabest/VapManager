...
void leftMenu::initUI()
{
    ...
    // Pre-warm layout/style so first open is instant (avoids cold stylesheet polish delay).
    userMenu->sizeHint();
    ...
    connect(userButton, &QToolButton::clicked, this, [this, userMenu](){
        // Fallback: InstantPopup normally handles press; keep manual popup for safety.
    ...
    QHBoxLayout *maintenanceTitleRow = new QHBoxLayout();
    maintenanceTitleRow->setContentsMargins(s(15), s(10), s(15), 0);
    maintenanceTitleRow->setSpacing(s(10));
    ...
    maintenanceTitle->setStyleSheet(QString(
        "background:transparent;font-family:Inter;font-size:%1px;font-weight:800;color:black;"
    ).arg(s(18)));
    maintenanceTitle->setAlignment(Qt::AlignLeft | Qt::AlignVCenter);
    maintenanceTitleRow->addWidget(maintenanceTitle);
    maintenanceTitleRow->addStretch();

    maintenanceOverdueCounter_ = new QLabel(QStringLiteral("Просрочено: 0"), rightUpcomingMaintenanceFrame);
    maintenanceOverdueCounter_->setAlignment(Qt::AlignCenter);
    ...
    maintenanceTitleRow->addWidget(maintenanceOverdueCounter_);
    rightUpcomingLayout->addLayout(maintenanceTitleRow);
    ...
    connect(listAgvInfo, &ListAgvInfo::bulkCompleteTasksRequested, this, &leftMenu::openBulkCompleteTasksDialog);
    ...
    connect(notifPollTimer, &QTimer::timeout, this, [this]() {
        updateNotifBadge();
        if (chatsPage && chatsPage->isVisible() && chatsStack_ && chatsStack_->currentIndex() == 0) {
    ...
    chatsPollTimer->setInterval(10000);
    ...
    dbReconnectTimer_ = new QTimer(this);
    dbReconnectTimer_->setInterval(15000);
    connect(dbReconnectTimer_, &QTimer::timeout, this, &leftMenu::tryReconnectDatabase);
    dbReconnectTimer_->start();
}
