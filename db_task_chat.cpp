...
{
    QSqlDatabase db = QSqlDatabase::database("main_connection");
    if (!db.isOpen()) { error = "БД недоступна"; return 0; }
    ...
    QString safeAgvId = agvId.trimmed().isEmpty() ? QStringLiteral("—") : agvId.trimmed();
...
{
    const QString u1 = user1.trimmed();
    const QString u2 = user2.trimmed();
    ...
    q.bindValue(":u2", u2);
    if (!optionalAgvId.trimmed().isEmpty())
        q.bindValue(":agv", optionalAgvId.trimmed());
...
static QSet<int> loadHiddenMessageIds(const QString &currentUser)
{
    QSet<int> hiddenIds;
    QSqlDatabase db = QSqlDatabase::database("main_connection");
    if (!db.isOpen() || currentUser.trimmed().isEmpty()) return hiddenIds;
    QSqlQuery h(db);
    h.prepare("SELECT message_id FROM task_chat_message_hidden WHERE username = :u");
    h.bindValue(":u", currentUser.trimmed());
    if (h.exec()) {
...
static TaskChatMessage rowToMessage(const QSqlQuery &q)
{
    TaskChatMessage m;
...
QVector<TaskChatMessage> getMessagesForThread(int threadId, const QString &currentUser)
{
    ...
    QSqlQuery q(db);
    if (currentUser.trimmed().isEmpty()) {
        ...
        q.bindValue(":u", currentUser.trimmed());
        q.bindValue(":id", threadId);
    }
    if (!q.exec()) return list;
