...
QDateTime notificationDndUntilValue()
{
    QSettings s(QStringLiteral("VapManager"), QStringLiteral("VapManager"));
    return s.value(QStringLiteral("notifications/dnd_until")).toDateTime();
}

void storeNotificationDndUntil(const QDateTime &when)
{
    QSettings s(QStringLiteral("VapManager"), QStringLiteral("VapManager"));
    s.setValue(QStringLiteral("notifications/dnd_until"), when);
    s.sync();
}

} // namespace

bool isNotificationDndActive()
{
    const QDateTime until = notificationDndUntilValue();
    return until.isValid() && until > QDateTime::currentDateTime();
}

QString notificationDndStatusText()
{
    if (!isNotificationDndActive())
        return QString();
    return QStringLiteral("Не беспокоить до %1")
        .arg(notificationDndUntilValue().toString(QStringLiteral("dd.MM.yyyy HH:mm")));
}

void setNotificationDndForOneHour()
{
    storeNotificationDndUntil(QDateTime::currentDateTime().addSecs(3600));
}

void setNotificationDndUntilEndOfShift()
{
    QDateTime now = QDateTime::currentDateTime();
    QDateTime shiftEnd(now.date(), QTime(18, 0));
    if (now >= shiftEnd)
        shiftEnd = shiftEnd.addDays(1);
    storeNotificationDndUntil(shiftEnd);
}

void clearNotificationDnd()
{
    QSettings s(QStringLiteral("VapManager"), QStringLiteral("VapManager"));
    s.remove(QStringLiteral("notifications/dnd_until"));
    s.sync();
}

QHash<int, int> unreadChatCountsByThread(const QString &username)
{
    ...
    QSqlQuery q(db);
    q.prepare("SELECT message FROM notifications WHERE target_user = :u AND is_read = FALSE AND message LIKE '%[chat:%'");
    q.bindValue(":u", u);
    if (!q.exec())
        return counts;

    static const QRegularExpression re(QStringLiteral("\\[chat:(\\d+)\\]"));
    while (q.next()) {
        const QRegularExpressionMatch m = re.match(q.value(0).toString());
        if (!m.hasMatch())
            continue;
        const int tid = m.captured(1).toInt();
        if (tid > 0)
            counts[tid] = counts.value(tid, 0) + 1;
    }
    return counts;
}

int notificationChatId(const QString &storedMessage)
{
    static const QRegularExpression re(QStringLiteral("\\[chat:(\\d+)\\]"));
...
QString notificationAgvId(const QString &storedMessage)
{
    static const QRegularExpression reAgv(QStringLiteral("AGV\\s*№?\\s*([A-Za-z0-9_\\-]+)"));
    QRegularExpressionMatch m = reAgv.match(storedMessage);
    if (m.hasMatch())
        return m.captured(1).trimmed();
    static const QRegularExpression reFor(QStringLiteral("для AGV\\s+([A-Za-z0-9_\\-]+)"),
                                            QRegularExpression::CaseInsensitiveOption);
    m = reFor.match(storedMessage);
    if (m.hasMatch())
        return m.captured(1).trimmed();
    return QString();
}
