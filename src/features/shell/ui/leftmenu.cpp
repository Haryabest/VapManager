...
void leftMenu::setScaleFactor(qreal factor)
{
    ...
    // Сохраняем черновик чата перед пересборкой UI
    if (activePage_ == ActivePage::Chats && embeddedChatWidget_) {
        activeChatDraft_ = embeddedChatWidget_->currentDraft();
        activeChatDraftFocused_ = embeddedChatWidget_->isDraftFocused();
    }
    ...
    maintenanceOverdueCounter_ = nullptr;
    listAgvInfo = nullptr;
...
void leftMenu::resizeEvent(QResizeEvent *event)
{
    ...
    // Масштаб только по ширине окна. Виртуальная клавиатура на планшете
    // уменьшает высоту, но не ширину — иначе каждое открытие/закрытие
    // клавиатуры пересобирает весь UI (initUI + restoreActivePage).
    const qreal factor = qMin<qreal>(1.0, width() / 1920.0);
