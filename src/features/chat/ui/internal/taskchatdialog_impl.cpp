...
void TaskChatWidget::setThreadId(int id, const QString &peerHint)
{
    ...
    cachedThreadLoadedId_ = 0;
    cachedThreadCreatedBy_.clear();
    cachedThreadRecipientUser_.clear();
...
QString TaskChatWidget::currentDraft() const
{
    return replyEdit_ ? replyEdit_->text() : QString();
}

bool TaskChatWidget::isDraftFocused() const
{
    return replyEdit_ ? replyEdit_->hasFocus() : false;
}

void TaskChatWidget::setDraft(const QString &text, bool setFocus)
{
    if (replyEdit_) {
        replyEdit_->setText(text);
        if (setFocus) {
            replyEdit_->setFocus();
        }
    }
...
void TaskChatWidget::setupUi()
{
    ...
    // Пока пользователь печатает в поле ответа, не дёргаем БД каждые 3с —
    // на удалённых клиентах это блокирует UI и мешает вводу. Возобновляем
    // polling, когда поле теряет фокус.
    if (replyEdit_) {
        connect(replyEdit_, &QLineEdit::textEdited, this, [this]() {
            if (liveRefreshTimer_) liveRefreshTimer_->stop();
        });
        connect(replyEdit_, &QLineEdit::editingFinished, this, [this]() {
            if (liveRefreshTimer_ && !liveRefreshTimer_->isActive()) liveRefreshTimer_->start();
        });
    }
...
void TaskChatWidget::applyRefreshedMessages(const QVector<TaskChatMessage> &msgs, bool fullReload, bool keepAtBottom)
{
    ...
    if (fullReload || peerUsername_.trimmed().isEmpty()) {
        ...
        // Кешируем метаданные треда, чтобы не делать лишних round-trip при отправке.
        cachedThreadLoadedId_ = threadState.id;
        cachedThreadCreatedBy_ = threadState.createdBy;
        cachedThreadRecipientUser_ = threadState.recipientUser;
        updatePeerHeaderMeta();
    }
...
void TaskChatWidget::sendReply()
{
    ...
    replyEdit_->clear();
    forceScrollToBottom_ = true;
    // Используем кешированные данные треда/имени, чтобы не делать 2 лишних
    // round-trip к удалённой БД на каждой отправке (главная причина лагов на
    // клиентах не на сервере).
    const QString fromName = !cachedSenderDisplayName_.isEmpty()
        ? cachedSenderDisplayName_
        : (cachedSenderDisplayName_ = userDisplayName(currentUser_));
    QString title = "Новое сообщение в чате";
    QString body = QString("[chat:%1] %2: %3").arg(threadId_).arg(fromName, notifBody.left(80));
    const QString createdBy = (cachedThreadLoadedId_ == threadId_)
        ? cachedThreadCreatedBy_ : QString();
    const QString recipientUser = (cachedThreadLoadedId_ == threadId_)
        ? cachedThreadRecipientUser_ : QString();
    if (!createdBy.isEmpty()) {
        if (currentUser_ == createdBy) {
            if (!recipientUser.isEmpty()) addNotificationForUser(recipientUser, title, body);
            else {
            ...
            addNotificationForUser(createdBy, title, body);
        }
    } else {
        // Кеш пуст (тред только что открыт, полный reload ещё не отработал) —
        // подгружаем по-старому, но один раз.
        ...
        cachedThreadLoadedId_ = t.id;
        cachedThreadCreatedBy_ = t.createdBy;
        cachedThreadRecipientUser_ = t.recipientUser;
    }
    DataBus::instance().triggerNotificationsChanged();
    replyToMessageId_ = 0;
