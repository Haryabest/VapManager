    ...
    : QDialog(parent), s(scale)
{
    ...
    // ===== NAME (AGV) =====
    //
    root->addWidget(makeTitle("Название AGV"));
    ...
    // ===== SERIAL (SN) =====
    //
    root->addWidget(makeTitle("Серийный номер (10 цифр)"));
    ...
    // Загружаем список моделей через твою функцию
    QStringList names = loadModelNames();
    ...
    // ===== ALIAS =====
    //
    root->addWidget(makeTitle("Заметка (необязательно)"));
    ...
    connect(addBtn, &QPushButton::clicked, this, [this](){
        ...
        QString finalName = "AGV-" + digits;
        if (!letters.isEmpty())
            finalName += "-" + letters.toUpper();
